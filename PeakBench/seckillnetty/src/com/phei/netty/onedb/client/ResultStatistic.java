...
public class ResultStatistic implements Runnable
{
	...
	public ResultStatistic(int queueSize, int readworkloadtime, 
			int writeworkloadtime, int readwriteinterval)
	{
		super();
				...
				if(sum>=3&&sum >presum)
				{
					for(int j=0;j<14;j++)
					{
						...
						else
						{
							...
							System.out.println("write time"+j+": "+wtime / resultStatistic.get(index).get(j).size());
							logger.info("write time"+j+": "+wtime / resultStatistic.get(index).get(j).size());
							wtime = 0;
						}
					...
					for(int j=14;j<20;j++)
					{
						...
						else
						{
							...
							System.out.println("read time"+j+": "+rtime / resultStatistic.get(index).get(j).size());
							logger.info("read time"+j+": "+rtime / resultStatistic.get(index).get(j).size());
							rtime = 0;
						}
						resultStatistic.get(index).get(j).clear();
					}
